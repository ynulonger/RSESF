...
class CustomImageSet(Dataset):
    def __init__(self, image_set='train', sample_weight=None, mask_channel=5, visualisation=False, d_name='Mosaics', transform=None):   # initial logic happens like transform
        ...
        if self._image_set == 'train':
            self._image_names = glob.glob(HOME_DIR + d_name + '/train-imgs/*.png')
            self._mask_names = glob.glob(HOME_DIR + d_name + '/train-gts/*.png')
...
def mapping(gt, channels):
    size = gt.shape
    mask = np.zeros([size[0], size[1], channels])
    labels = np.unique(gt)
...
class GlaS(Dataset):
    def __init__(self, image_set='train', sample_weight=None, mask_channel=2, transform=None):
        self.sample_weight=sample_weight
        self._image_set = image_set
        self.mask_channel = mask_channel
        self.transform  = transform
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            transformed = self.transform(image=image, mask=mask)
            # print('mask shape:', transformed["mask"].shape, 'img shape:', transformed["image"].shape)
            img = transformed["image"]
            mask = transformed["mask"].permute(2,0,1)
