...
def evaluate(criterion, net, loader, classes, n_scales=3):
    ...
    for batch in loader:
        with torch.no_grad():
            ...
            pred_list = net(imgs)
            temp_loss, temp_loss_list = compute_loss(criterion, pred_list, true_masks, net, n_scales)
            loss += temp_loss
...
def train_net(net, train_dataset, val_dataset, epochs, batch_size, lr, \
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            loss, loss_list = compute_loss(criterion, pred_list, true_masks, net, branches)
        ...
        val_cm, val_loss, val_hidden_loss       = evaluate(criterion, net, val_loader, classes, branches)
...
def get_args():
    ...
    parser.add_argument('-d', '--data', dest='data', type=str, default='BCSS',
                        help='dataset')
    parser.add_argument('-s', '--run', dest='run', type=str, default='run',
                        help='srf')
...
if __name__ == '__main__':
    ...
    if dataset=='CRAG':
        classes = 2
        n_channels=3
        train_dataset = CRAG('train', sample_weight=None, mask_channel=classes, transform=CRAG_train_transform)
        test_dataset  = CRAG('test', sample_weight=None, mask_channel=classes,  transform=CRAG_test_transform)
    elif dataset=='GlaS':
        classes = 2
        n_channels=3
        train_dataset = GlaS('train', sample_weight=None, mask_channel=classes, transform=GlaS_train_transform)
        test_dataset  = GlaS('test', sample_weight=None, mask_channel=classes,  transform=GlaS_test_transform)

    elif 'Mosaic' in dataset:
        img_size= 512
        classes = 5
        n_channels=1
        train_dataset = CustomImageSet('train', sample_weight=None, mask_channel=classes, transform=Mosaic_train_transform, d_name=dataset)
        test_dataset  = CustomImageSet('test', sample_weight=None, mask_channel=classes,  transform=Mosaic_test_transform, d_name=dataset)

    # if dataset=='CRAG':
    #     classes = 2
    #     n_channels=3
    #     train_dataset = CRAG('train', sample_weight=None, mask_channel=classes)
    #     test_dataset  = CRAG('test', sample_weight=None, mask_channel=classes)
    # elif dataset=='GlaS':
    #     classes = 2
    #     n_channels=3
    #     train_dataset = GlaS('train', sample_weight=None, mask_channel=classes)
    #     test_dataset  = GlaS('test', sample_weight=None, mask_channel=classes)

    # elif 'Mosaic' in dataset:
    #     img_size= 512
    #     classes = 5
    #     n_channels=1
    #     train_dataset = CustomImageSet('train', sample_weight=None, mask_channel=classes, d_name=dataset)
    #     test_dataset  = CustomImageSet('test', sample_weight=None, mask_channel=classes, d_name=dataset)
    ...
    net = UNet_R(n_channels=n_channels, n_classes=classes, filters=args.n_filters, init_order=args.order, bilinear=True, \
                     ...
                     n_scales=args.n_scales, rotation_size=1, learn_sigma=True, k=2.5, scale_offset=scale_offset).cuda()
    ...
    net = train_net(net=net, train_dataset= train_dataset, val_dataset=test_dataset, epochs=args.epochs, 
                    ...
                    save_name=args.run+'_'+str(args.lr)+'_'+str(args.batch_size)+'_'+str(args.epochs)+'_'+args.model+'_order='+\
                    str(args.order)+'_scale='+str(args.n_scales)+'_rotation='+str(args.n_rotation), branches=args.n_scales)
