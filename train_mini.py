...
#     [
#         A.ShiftScaleRotate(shift_limit=0., scale_limit=1.0, rotate_limit=[0, 360], p=0.99),
#         ToTensorV2(),
#     ]
# )
...
def get_args():
    ...
    parser.add_argument('-b', '--batch_size', dest='batch_size', type=int, default=16,
                        help='batch size')
    parser.add_argument('-m', '--model', dest='model', type=str, default='E2CNN',
                        help='model')
    ...
    parser.add_argument('-k', '--k_size', dest='k_size', type=int, default=5,
                        help='k_size')
    parser.add_argument('-n', '--n_filters', dest='n_filters', type=int, default=64,
                        help='num of filters')
    ...
    parser.add_argument('-o', '--order', dest='order', type=int, default=2,
                        help='order')
    parser.add_argument('-f', '--k_factor', dest='k_factor', type=float, default=2,
                        help='order')
...
if __name__ == '__main__':
    ...
    dir_checkpoint = 'checkpoints_mini/'+dataset+'/'
    # dir_checkpoint = 'checkpoints_mini_aug/'+dataset+'/'
    ...
    if dataset=='CRAG':
        ...
        classes = 2
        n_channels=3
        train_dataset = GlaS('train',  mask_channel=classes)
        test_dataset  = GlaS('test',  mask_channel=classes)
    elif 'Mosaic' in dataset:
        img_size= 512
        classes = 5
        n_channels=1
        # train_dataset = CustomImageSet('train',  mask_channel=classes, d_name=dataset, transform=Mosaic_train_transform)
        # test_dataset  = CustomImageSet('test',  mask_channel=classes,  d_name=dataset, transform=Mosaic_test_transform)
        train_dataset = CustomImageSet('train',  mask_channel=classes, d_name=dataset)
        test_dataset  = CustomImageSet('test',  mask_channel=classes,  d_name=dataset)

    if args.model=='RST':
        # net = UNet_R_Mini(n_channels=n_channels, n_classes=classes, filters=args.n_filters, bilinear=True, kernel_size=args.k_size).cuda()
        net = UNet_R_Mini(n_channels=n_channels, n_classes=classes, filters=args.n_filters, init_order=args.order, bilinear=True, \
                    init_scale=0.99, angles=[torch.tensor(0+i*math.pi/(args.n_rotation/2)) for i in range(args.n_rotation)],\
                    n_scales=1, rotation_size=1, learn_sigma=True, k=args.k_factor, selection=0, lower_bound=args.lower_bound).cuda()
        save_name='lr_'+str(args.lr)+'_bn_'+str(args.batch_size)+'_epoch_'+str(args.epochs)+'_'+args.model+'_k_'+str(args.k_factor)+'_lower_bound_'+str(args.lower_bound)+'_rotation_'+str(args.n_rotation)
    ...
    print(f'------k_factor:{args.k_factor},-------lower_bound:{args.lower_bound}-------------------------')
    print('num of params:',get_num_parameters(net))
    net.to(device=device)

    torch.save(net.state_dict(), dir_checkpoint + f'init_{save_name}.pth')
