...
def train_net(net, device, train_dataset, val_dataset, epochs, batch_size, lr, model, dir_checkpoint, loss_fn, classes, save_name):
    ...
    for epoch in range(epochs):
        ...
        wandb.log({'train_loss': train_loss, 'train_miou':train_miou.mean()})
        wandb.log({'test_loss': val_loss, 'test_miou':val_miou.mean()})
...
if __name__ == '__main__':
    ...
    if dataset=='CRAG':
        ...
        classes = 2
        n_channels=3
        train_dataset = CRAG('train', transform=CRAG_train_transform)
        test_dataset  = CRAG('test', transform=CRAG_test_transform)
    elif dataset=='GlaS':
        img_size= 512
        classes = 2
        n_channels=3
        train_dataset = GlaS('train', transform=GlaS_train_transform)
        test_dataset  = GlaS('test', transform=GlaS_test_transform)
    elif 'Mosaic' in dataset:
        img_size= 512
        classes = 5
        n_channels=1
        train_dataset = CustomImageSet('train', transform=Mosaic_train_transform, d_name=dataset)
        test_dataset  = CustomImageSet('test', transform=Mosaic_test_transform, d_name=dataset)
    ...
    project = args.model+'-'+args.data+'-Aug'
