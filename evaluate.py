...
if __name__ == "__main__":
    ...
    if args.name=='CNN':
        ...
        if args.mode=='predict':
            net = UNet_R_Pool(n_channels=n_channels, n_classes=classes, filters=args.n_filters, init_order=2, bilinear=True, \
                     init_scale=0.99, angles=[torch.tensor(0+i*2*math.pi/args.n_rotation) for i in range(args.n_rotation)],\
                     n_scales=4, rotation_size=1, learn_sigma=True).cuda()
